...
def main():
    ...
    model = Net(args.hsi_bands,  args.sar_bands, args.dsm_bands, args.hidden_size, args.patch_size, args.num_class).eval().to(device)
    ...
    for j in range(int((len(idx) - (len(idx) % batch)) / batch + 1)):
        ...
        prediction = model(tmphsi, tmpsar, tmpdsm)
