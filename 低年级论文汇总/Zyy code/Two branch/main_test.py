...
def main():
    ...
    data_dsm = scio.loadmat(args.root + args.dataset + '/data_DSM.mat')['data']
    ...
    model = Net(args.hsi_bands, args.sar_bands, args.dsm_bands, args.patch_size, args.num_class).eval().to(device)
    model.load_state_dict(torch.load('./checkpoints/model.pth'))

    margin = (args.patch_size - 1) // 2
    data_hsi = addZeroPadding(data_hsi, margin)
    data_sar = addZeroPadding(data_sar, margin)
    data_dsm = addZeroPadding(data_dsm, margin)
    ...
    for j in range(int((len(idx) - (len(idx) % batch)) / batch + 1)):
        ...
        tmphsip = np.array([data_hsi[idx[j * batch + i]:idx[j * batch + i] + 1,
                           idy[j * batch + i]:idy[j * batch + i] + 1, :] for i in range(num_cat)])
        tmpsar = np.array([data_sar[idx[j * batch + i] - margin:idx[j * batch + i] +
                                                                margin + 1,
                           idy[j * batch + i] - margin:idy[j * batch + i] + margin + 1, :] for i in range(num_cat)])
        tmpdsm = np.array([data_dsm[idx[j * batch + i] - margin:idx[j * batch + i] +
                                                                margin + 1,
                           idy[j * batch + i] - margin:idy[j * batch + i] + margin + 1, :] for i in range(num_cat)])
        tmphsi = torch.FloatTensor(tmphsi.transpose(0, 3, 1, 2)).to(device)
        tmphsip = torch.FloatTensor(tmphsip.transpose(0, 3, 1, 2)).to(device)
        tmpsar = torch.FloatTensor(tmpsar.transpose(0, 3, 1, 2)).to(device)
        tmpdsm = torch.FloatTensor(tmpdsm.transpose(0, 3, 1, 2)).to(device)

        prediction = model(tmphsi, tmphsip, tmpsar, tmpdsm)
