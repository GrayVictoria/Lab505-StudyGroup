...
def train(model, device, train_loader, optimizer, epoch, slim_params, bn_threshold):
    model.train()
    total_loss = 0
    for i, (inputs_1, inputs_2, inputs_3, inputs_4, labels) in enumerate(train_loader):
        inputs_1, inputs_2, inputs_3, inputs_4 = inputs_1.to(device), inputs_2.to(device), inputs_3.to(device), inputs_4.to(device)
        labels = labels.to(device)

        optimizer.zero_grad()
        outputs = model(inputs_1, inputs_2, inputs_3, inputs_4)
...
def test(model, device, test_loader):
    model.eval()
    count = 0
    for inputs_1, inputs_2, inputs_3, inputs_4, labels in test_loader:
        inputs_1, inputs_2, inputs_3, inputs_4 = inputs_1.to(device), inputs_2.to(device), inputs_3.to(device), inputs_4.to(device)
        outputs = model(inputs_1, inputs_2, inputs_3, inputs_4)
...
def main():
    ...
    model = Net(args.hsi_bands,  args.sar_bands, args.dsm_bands, args.patch_size, args.num_class).to(device)
