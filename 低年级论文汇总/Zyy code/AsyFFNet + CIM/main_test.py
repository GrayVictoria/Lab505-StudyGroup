...
def main():
    ...
    data_dsm = scio.loadmat(args.root + args.dataset + '/data_msi.mat')['data']
    ...
    model = Net(args.hsi_bands,  args.sar_bands, args.msi_bands, args.hidden_size, args.patch_size, args.num_class).eval().to(device)
    ...
    for j in range(int((len(idx) - (len(idx) % batch)) / batch + 1)):
        ...
        prediction, _, _, _ = model(tmphsi, tmpsar, tmpdsm)
