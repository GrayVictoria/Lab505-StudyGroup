...
def main():
    ...
    model = Net(args.hsi_bands, args.sar_bands, args.dsm_bands, args.hidden_size, Bottleneck, args.num_parallel,
                args.num_reslayer, args.num_class, args.bn_threshold).to(device)
    ...
    for j in range(int((len(idx) - (len(idx) % batch)) / batch + 1)):
        ...
        prediction, _ = model(tmphsi, tmpsar, tmpdsm)
