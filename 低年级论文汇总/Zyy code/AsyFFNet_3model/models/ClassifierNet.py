...
class Exchange(nn.Module):
    ...
    def forward(self, x, bn, bn_threshold):
        ...
        x1[:, bn1 >= bn_threshold] = x[0][:, bn1 >= bn_threshold]
        x1[:, bn1 < bn_threshold] = x[2][:, bn1 < bn_threshold]
        x3[:, bn3 >= bn_threshold] = x[2][:, bn3 >= bn_threshold]
        x3[:, bn3 < bn_threshold] = x[0][:, bn3 < bn_threshold]

        x2[:, bn2 >= bn_threshold] = x[1][:, bn2 >= bn_threshold]
        x2[:, bn2 < bn_threshold] = x[2][:, bn2 < bn_threshold]
        x3[:, bn3 >= bn_threshold] = x[2][:, bn3 >= bn_threshold]
        x3[:, bn3 < bn_threshold] = x[1][:, bn3 < bn_threshold]
        return [x1, x2, x3]
...
class Net(nn.Module):
    def __init__(self, hsi_channels, sar_channels, dsm_channels, hidden_size, block, num_parallel, num_reslayer=2, num_classes=7, bn_threshold=2e-2):
        ...
        self.conv_11 = nn.Sequential(nn.Conv2d(sar_channels, hidden_size, 1, bias=False),
            nn.BatchNorm2d(hidden_size))
        self.conv_22 = nn.Sequential(nn.Conv2d(dsm_channels, hidden_size, 1, bias=False),
            nn.BatchNorm2d(hidden_size))
        ...
        self.Attention = External_attention(hidden_size * num_parallel)
        self.SCConv = SCConv(hidden_size * num_parallel, hidden_size)
    ...
    def forward(self, x, y, z):
        ...
        z = F.relu(self.conv_22(z)).unsqueeze(0)
        x = torch.cat((x, y, z), 0)
